...
if($_POST['submitok']==1){
	...
	$sql = "SELECT `imo` FROM `_xvas_shipdata_wet_user` WHERE `imo`='".$print['MAIN']['IMO_NUMBER']."' LIMIT 0,1";
	$r = dbQuery($sql);
	
	if(!trim($r[0]['imo'])){
		$sql = "INSERT INTO `_xvas_shipdata_wet_user` (`imo`, `data`, `by_user`, `dateadded`, `dateupdated`) VALUES('".mysql_escape_string($print['MAIN']['IMO_NUMBER'])."', '".mysql_escape_string($data)."', '".mysql_escape_string($by_user)."', NOW(), NOW())";
		dbQuery($sql, $link);
	}else{
		$sql = "UPDATE `_xvas_shipdata_wet_user`
...
if(!trim($_GET['imo']) && !isset($_GET['msg'])){
	...
	$sql = "SELECT * FROM _xvas_shipdata_wet_user WHERE imo='".$_GET['imo']."' LIMIT 0,1";
	$r = dbQuery($sql);
	
	if($r[0]['imo']==''){
		$sql = "SELECT * FROM _xvas_shipdata_wet WHERE imo='".$_GET['imo']."' LIMIT 0,1";
		$r = dbQuery($sql);
		...
		$sql1 = "select `imo` from `_xvas_shipdata_wet_user` where `by_user` in ( 
