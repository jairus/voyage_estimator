...
if($t_ships){
	...
	<table id='pgreens11' width='1000px' style='border:1px solid #000;'>
		<tr>
			...
			<th width='80px' style='background:#BCBCBC; color:#333333;'><div style='padding:5px;'>GT</div></th>
		...
		if($nmessage['dely']==$destination_port){
			if(date('Ymd', strtotime($destination_port_from))<=date('Ymd', strtotime($nmessage['delydate_from'])) && date('Ymd', strtotime($destination_port_to))>=date('Ymd', strtotime($nmessage['delydate_from']))){
				if($counter==1){
					...
					<table id='porange11' width='1000px' style='border:1px solid #000;'>
						<tr>
							<th width='20px' style='text-align:center; background-color:#ccc;'><div style='padding:5px;'><input type='checkbox' onclick=\"checkAll('porange11', this)\" ></div></th>
							<th width='20px' style='background-color:#ccc; text-align:center;'><div style='padding:5px;'><img src='images/icon_book.png' border='0' /></div></th>
							<th width='125px' style='background:#BCBCBC; color:#333333;'><div style='padding:5px;'>Name</div></th>
							<th width='100px' style='background:#BCBCBC; color:#333333; text-align:right;'><div style='padding:5px;'>Destination</div></th>
							<th width='110px' style='background:#BCBCBC; color:#333333;'><div style='padding:5px;'>ETA</div></th>
				...
				if(trim($xvas['data'])){
					...
					if(trim($status)!="DEAD"){
						...
						if(!empty($bupdate) || !empty($oupdate) || !empty($eupdate)){
							if((time()-$delydate_to)<(60*60*24*15) || (time()-strtotime(date('M d, Y', strtotime($eupdate[0]['from_time']))))<(60*60*24*15) || !empty($oupdate)){
								$updates = "<img src='images/icon_dropdown_warning_shore.png' width='20' height='18' style='cursor:pointer;' onclick=\"expand('s_drop2_".$i_broker."', '".$ships['xvas_imo']."', 'shore');\" id='s_drop2_".$i_broker."_img' />";
							}else{
								$updates = "<img src='images/icon_dropdown.png' width='20' height='18' style='cursor:pointer;' onclick=\"expand('s_drop2_".$i_broker."', '".$ships['xvas_imo']."', 'shore');\" id='s_drop2_".$i_broker."_img' />";
							}
						}else{
							$updates = "<img src='images/icon_dropdown.png' width='20' height='18' style='cursor:pointer;' onclick=\"expand('s_drop2_".$i_broker."', '".$ships['xvas_imo']."', 'shore');\" id='s_drop2_".$i_broker."_img' />";
						}
						...
						<tr style='width:992px; background:#fff;'>
							<td colspan='14' id='s_drop2_".$i_broker."' style='display:none;'></td>
						</tr>";
					}
				}
