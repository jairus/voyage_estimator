...
if(trim($_GET['port_name'])){
	...
	if($_GET['p_vessel_type']){
		...
		$sql = "SELECT * FROM `_xvas_siitech_cache_wet` WHERE `siitech_destination`='".$port_name."' AND ".$sqlext." AND `siitech_eta` BETWEEN '".date('Y-m-d', strtotime($_GET['date_from']))."' AND '".date('Y-m-d', strtotime($_GET['date_to']))."' ORDER BY siitech_eta DESC";
		...
		if(trim($t)){
			...
			for($i=0; $i<$t; $i++){
				...
				$sql_xvas = "SELECT `name` FROM `_xvas_parsed2` WHERE `imo`='".$ships[$i]['xvas_imo']."' ORDER BY dateupdated DESC LIMIT 0,1";
