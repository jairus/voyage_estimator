...
if($user['dry']==1){
	$sql_pos  = "SELECT * FROM `_xvas_siitech_cache` WHERE `xvas_imo`='".$imo."' ORDER BY `dateupdated` DESC LIMIT 0,1";
	$xvas_pos = dbQuery($sql_pos);
	$xvas_pos = $xvas_pos[0];
}elseif($user['dry']==0){
	$sql_pos  = "SELECT * FROM `_xvas_siitech_cache_wet` WHERE `xvas_imo`='".$imo."' ORDER BY `dateupdated` DESC LIMIT 0,1";
	$xvas_pos = dbQuery($sql_pos);
	$xvas_pos = $xvas_pos[0];
}
...
$sql_bi  = "SELECT * FROM `_messages` WHERE `imo`='".$imo."' AND type='network' AND user_email in ( 
...
$siitech_latitude = $xvas_pos['siitech_latitude'];
$siitech_longitude = $xvas_pos['siitech_longitude'];
$xstring = str_replace("\n", "", $xstring);
