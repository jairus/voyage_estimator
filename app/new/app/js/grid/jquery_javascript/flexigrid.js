	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
							 	...
							 	function ()
									{
										...
										td.innerHTML = '<input type="text" name="inline[]" class="inline-edit" value="'+row.cell[idx]+'">';
										}
										else{
			...
			doSearch: function () {
				...
				var qs = new Array();
				$( '.colFilter', g.hDiv ).each( function(){
					if( $(this).val() != '' ){
						var attrname = $(this).attr('name').replace(' ','_');
						qs.push( attrname+'='+escape($(this).val()) );
					}
				});
							...
							.dblclick(
								function (e) 
									{ 
										...
										if (p.onRowSelect_dblclick){
											p.onRowSelect_dblclick(e,$(this)); // *** I added *** 
										} 
									}
							)
							.mousedown(
			...
			for (i=0;i<p.colModel.length;i++)
				{
					...
					cm.searchable = ( cm.searchable == undefined ) ? true : cm.searchable; // hack
					cm.editable = ( cm.editable == undefined ) ? false : cm.editable; // hack
					...
					if (cm.searchable) 
						$(th).addClass('searchable');// hack

					if (cm.editable) 
						$(th).addClass('editable');// hack
		...
		//set gDiv
		g.gDiv.className = 'flexigrid docs';
		...
		//fDiv2.className = 'fDiv2';		
		//fDiv2.innerHTML = 'adasdasdasd';
		//$(g.fDiv).append(fDiv2);
		//$(g.fDiv).append("<div style='clear:both'></div>");
...
// hacked				
						...
						var wat = $(this).attr('abbr').toLowerCase();							
						var vval = p.defaultSearch[wat] != undefined ? p.defaultSearch[wat] : '' ;							
						if( !p.useInlineSearch )
						thdiv.innerHTML = this.innerHTML;								
						else
						thdiv.innerHTML = this.innerHTML + '<br /><input type="text" size="4" value="'+vval+'" name="'+ wat +'" class="colFilter"/>';	
						}
						else{
						thdiv.innerHTML = this.innerHTML;									
						}						
...
//									alert( g.bDiv.offsetTop );
//									$(g.nDiv).css({top:g.bDiv.offsetTop});
// hacked
									bTop = -22 + g.bDiv.offsetTop;
									$(g.nDiv).css({top:bTop});
...
//						var bLabel = this.innerHTML.split('/<[br|BR\/]+>/');
						var bLabel = this.innerHTML.split(/<[br|BR\/]+>/);
						bLabel = bLabel[0].split(/<[br|BR\/]+>/);						

						//this.innerHTML
						$('tbody',g.hDiv).append('<tr><td class="ndcol1"><input type="checkbox" '+ chk +' class="togCol" value="'+ cn +'" /></td><td class="ndcol2">'+bLabel[0]+'</td></tr>');
						cn++;
					}
			);
...
// */*/*/
		if( !p.defaultSearch.empty )
			g.doSearch();	
		
		$('.colFilter', g.hDiv ).keydown(function(e){if(e.keyCode==13) g.doSearch()});
