...
if($t){
	...
	<table id='p_pgreens1' width='1000px' style='border:1px solid #000;'>
		<tr>
			...
			<th width='170px' style='background:#BCBCBC; color:#333333; text-align:right;'><div style='padding:5px;'>Open Port</div></th>
						...
						if(trim($status)!="DEAD"){
							...
							if(!empty($bupdate) || !empty($oupdate) || !empty($eupdate)){
								if((time()-$delydate_to)<(60*60*24*15) || (time()-strtotime(date('M d, Y', strtotime($eupdate[0]['from_time']))))<(60*60*24*15) || !empty($oupdate)){
									$updates = "<img src='images/icon_dropdown_warning_shore.png' width='20' height='18' style='cursor:pointer;' onclick=\"expand('p_drop1_".$i."', '".$ships['IMO #']."', 'shore');\" id='p_drop1_".$i."_img' />";
								}else{
									$updates = "<img src='images/icon_dropdown.png' width='20' height='18' style='cursor:pointer;' onclick=\"expand('p_drop1_".$i."', '".$ships['IMO #']."', 'shore');\" id='p_drop1_".$i."_img' />";
								}
							}else{
								$updates = "<img src='images/icon_dropdown.png' width='20' height='18' style='cursor:pointer;' onclick=\"expand('p_drop1_".$i."', '".$ships['IMO #']."', 'shore');\" id='p_drop1_".$i."_img' />";
							}
							...
							echo "<tr style='background:#e5e5e5;'>
								...
								echo "<td style='text-align:center;'><div style='padding:5px;'><a onclick=\"getHistory('".$ships['IMO #']."', 'openport', '".$details."', 'SHIPS WITH AIS DESTINATIONS & ETA');\" alt='Last Port History' title='Last Port History' class='history_link'><img src='images/icon_plusdown_warning.png' border='0' /></a></div</td>
