...
if($_GET['search']){
	...
	$sql = "select * from  _xvas_parsed2 where imo <> '' and (imo like '%".mysql_escape_string($search)."%' or name like '%".mysql_escape_string($search)."%') limit 20";
	...
	for($i=0; $i<$t; $i++){
		$sql2 = "select * from _xvas_shipdata where imo <> '' and imo='".trim($r[$i]['imo'])."' limit 1";
		$r2 = dbQuery($sql2);
...
$(function(){
	...
	$(".c31").autocomplete({
		...
		source: function(req, add){
			...
			$.getJSON("ajax_voyage_estimator_wet.php?port=1", req, function(data) {
	...
	$(".i35").autocomplete({
		...
		source: function(req, add){
			...
			//pass request to server
			$.getJSON("ajax_voyage_estimator_wet.php?sf=1", req, function(data) {
	...
	$("#ship").autocomplete({
		...
		source: function(req, add){
			...
			//pass request to server
			$.getJSON("ajax_voyage_estimator_wet.php?search=1", req, function(data) {
				//create array for response objects
				var suggestions = [];
				var imos = [];
...
function showShipDetails(imo){
	...
	jQuery.ajax({
		type: 'POST',
		url: "search_ajax1ve_wet.php?imo="+gimo,
		data:  '',
...
function showShipDetails2(imo){
	...
	jQuery("#shipdetailiframe")[0].src='misc/ship_data_update_wet.php?imo='+gimo;
...
function ballastDistCalc(tmp, to, from, triggerajax){
	...
	if(to!=tox||from!=fromx||!distance||triggerajax){
		...
		jQuery.ajax({
			type: 'POST',
			url: "ajax_voyage_estimator_wet.php?dc=1&from="+from+"&to="+to,
			data:  '',
...
function autoSave(){
	...
	jQuery.ajax({
		...
		url: "ajax_voyage_estimator_wet.php?autosave=1",
