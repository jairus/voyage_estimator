...
if(trim($t)){
		...
		if($ships[$i]['imo']!=$ships[$i-1]['imo']){
			...
			if(trim($ship_exist['data'])){
				...
				if(trim($status)!="DEAD"){
					...
					$sql = "SELECT * FROM `_xvas_siitech_cache_wet` WHERE xvas_imo='".$ships[$i]['imo']."' AND satellite='0' ORDER BY dateupdated DESC";
	...
	if($t2){
		for($i2=0; $i2<$t2; $i2++){
			...
			$sql = "SELECT * FROM `_xvas_siitech_cache_wet` WHERE id='".$ship['id']."' AND satellite='0' ORDER BY dateupdated DESC";
			$siitech_ships = dbQuery($sql, $link);
			//END
